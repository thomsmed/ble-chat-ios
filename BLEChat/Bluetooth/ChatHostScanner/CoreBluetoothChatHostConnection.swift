...
final class CoreBluetoothChatHostConnection: NSObject {
    ...
    private func subscribeToReactionsCharacteristic() {
        guard
            ...
            let chatService = peripheral.services?.first(
                where: { service in
                    service.uuid == AssignedNumbers.chatService
                }
            ),
            let outgoingReactionsCharacteristic = chatService.characteristics?.first(
                where: { characteristic in
    ...
    private func fetchL2CAPChannelSMP() {
        guard
            ...
            let l2capChannelCharacteristic = chatService.characteristics?.first(
                where: { characteristic in
...
// MARK: Public interface on CoreBluetoothChatHostConnection

extension CoreBluetoothChatHostConnection {
    func connected(to peripheral: CBPeripheral) {
        peripheral.delegate = self
        peripheral.discoverServices([
            // We are only interested in discovering our custom Chat Service service.
...
extension CoreBluetoothChatHostConnection: CBPeripheralDelegate {
    func peripheral(
        ...
        services.forEach { service in
            // Attempt to discover all characteristics for all services.
    ...
    func peripheral(
        ...
        characteristics.forEach { characteristic in
            // Attempt to discover all descriptors for all characteristics.
    ...
    func peripheral(
        ...
        guard invalidatedServices.contains(
            where: { $0.uuid == AssignedNumbers.chatService }
        ) else { return }
    ...
    func peripheral(
        ...
        // In this application we'll just ignore it...
        if let error = error {
            print(error)
        }
    }
    ...
    func peripheral(
        ...
        // This method is called if a read is initiated by the central,
        // or when the peripheral has notified/indicated that a new value is available.
        if characteristic.uuid == AssignedNumbers.chatServiceL2CAPPSMCharacteristic {
            ...
            guard
                ...
                let reaction = String(data: data, encoding: .utf8)
            else {
                return
            }
    ...
    func submit(message: String) {
        serialQueue.async {
            ...
            data.withUnsafeBytes { ptr in
                guard let startOfData = ptr.baseAddress?.assumingMemoryBound(
                    to: UInt8.self
                ) else { return }
    ...
    func submit(reaction: String) {
        serialQueue.async {
            guard
                ...
                let inboxCharacteristic = chatService.characteristics?.first(
                    where: { characteristic in
