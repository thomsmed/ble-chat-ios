...
final class CoreBluetoothChatHost: NSObject {
    ...
    private lazy var stateSubject = CurrentValueSubject<ChatHostState, Never>(
        peripheralManager.state.asCHState
    )
...
extension CoreBluetoothChatHost: CBPeripheralManagerDelegate {
    ...
    func peripheralManager(
        ...
        if let error = error {
            ...
            peripheral.add(ChatServiceFactory.makeChatService(with: PSM))
    ...
    func peripheralManager(
        ...
        request.value = lastOutgoingReaction.data(using: .utf8)?.subdata(
            in: request.offset..<lengthOfLastOutgoingReaction
        )
    ...
    func peripheralManager(
        ...
        // It should be safe to assume there is at least one incoming request.
    ...
    func peripheralManager(
        ...
        // Useful if you want to track wether you should update a vale or not.
        // E.g. if you do not want to update a value that has no subscribers to it.
    ...
    func stream(_ stream: Stream, handle eventCode: Stream.Event) {
        switch eventCode {
            ...
            case .hasBytesAvailable:
                ...
                let data = Data(buffer: UnsafeMutableBufferPointer(
                    start: buffer, count: totalNumberOfBytesRead
                ))
